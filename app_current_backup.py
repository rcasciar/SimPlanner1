...
import tempfile
import openpyxl
from import_function import importa_giacenze_da_excel
...
def salva_dati_sessione():
    ...
    dati_sessione = {
        ...
        "device_giacenze": st.session_state.get("device_giacenze", []),
        "device_requisiti_lab": st.session_state.get("device_requisiti_lab", []),
    }
...
def carica_dati_sessione():
    ...
    try:
        if os.path.exists('temp_files/sessione.json'):
            ...
            # Carica dati dell'inventario
            if "device_giacenze" in dati and isinstance(dati["device_giacenze"], list):
                st.session_state.device_giacenze = dati["device_giacenze"]
            if "device_requisiti_lab" in dati and isinstance(dati["device_requisiti_lab"], list):
                st.session_state.device_requisiti_lab = dati["device_requisiti_lab"]
    except Exception as e:
...
# Inizializza le variabili di sessione per l'inventario
if 'device_giacenze' not in st.session_state:
    st.session_state.device_giacenze = []

if 'device_requisiti_lab' not in st.session_state:
    st.session_state.device_requisiti_lab = []
...
# Interfaccia utente
st.title("SimPlanner - Sistema di Programmazione Laboratori")

# Sidebar con informazioni e manuali
st.sidebar.title("SimPlanner")
st.sidebar.image("logo_simplanner.svg", width=200)
st.sidebar.markdown("Sistema Avanzato di Programmazione Laboratori")
st.sidebar.markdown("---")
st.sidebar.markdown("### Autore:")
st.sidebar.markdown("Dott. Mario Rossi")
st.sidebar.markdown("C.d.L. Infermieristica - Università di Milano")
st.sidebar.markdown("v1.2.0 - ©2024")
...
with col7:
    st.button("Inventario", on_click=vai_a_sezione, args=('Inventario',), use_container_width=True)
...
if st.session_state.sezione_corrente == 'Home':
    ...
    6. **Inventario**: Gestisci l'inventario dei materiali e le loro associazioni ai laboratori
    ...
    with col1:
        ...
        st.write(f"Materiali inventario: {len(st.session_state.device_giacenze)}")
        
        # Conteggia materiali sotto la soglia di 50 unità
        materiali_sotto_soglia = len([item for item in st.session_state.device_giacenze if item.get('quantita', 0) < 50])
        if materiali_sotto_soglia > 0:
            st.warning(f"Materiali sotto scorta minima: {materiali_sotto_soglia}")
    ...
    with col2:
        ...
        st.write(f"Gruppi a Capacità Ridotta: {n_gruppi_ridotti}")
    ...
    if st.session_state.laboratori:
        ...
        with col2:
            st.write("Configurazione Gruppi a Capacità Ridotta")
            ...
            if st.button("Genera Gruppi Ridotti"):
                ...
                if not labs_ridotti:
                    ...
                    st.success("Gruppi a capacità ridotta generati con successo!")
                    salva_dati_sessione()
...
elif st.session_state.sezione_corrente == 'Inventario':
    section_header("Gestione Inventario Materiali", "section-inventory")
    
    # Creazione di tre tab per diverse funzionalità
    tab1, tab2, tab3 = st.tabs(["Giacenze", "Materiali per Laboratorio", "Importa Materiali"])
    
    # Tab 1: Giacenze (inventario generale)
    with tab1:
        st.subheader("Giacenze Materiali")
        st.write("In questa sezione puoi visualizzare e modificare l'inventario dei materiali disponibili.")
        
        # Aggiungi materiale manualmente
        with st.expander("Aggiungi Nuovo Materiale", expanded=False):
            col1, col2, col3 = st.columns(3)
            with col1:
                nuovo_codice = st.text_input("Codice Prodotto", key="new_codice")
            with col2:
                nuovo_nome = st.text_input("Nome Prodotto", key="new_nome")
            with col3:
                nuova_quantita = st.number_input("Quantità", min_value=0, value=1, step=1, key="new_quantita")
            
            if st.button("Aggiungi Materiale"):
                if nuovo_codice and nuovo_nome:
                    # Verifica se il codice esiste già
                    codici_esistenti = [item["codice"] for item in st.session_state.device_giacenze]
                    
                    if nuovo_codice in codici_esistenti:
                        st.warning(f"Il codice '{nuovo_codice}' esiste già. Usa l'opzione di modifica per aggiornarlo.")
                    else:
                        ...
                        st.success(f"Materiale '{nuovo_nome}' aggiunto con successo!")
                        salva_dati_sessione()
                else:
                    st.error("Codice e Nome sono campi obbligatori.")
        
        # Tabella con tutti i materiali in inventario
        if st.session_state.device_giacenze:
            df_giacenze = pd.DataFrame(st.session_state.device_giacenze)
            
            # Evidenzia le giacenze sotto la soglia di 50 unità
            def highlight_low_inventory(row):
                if row['quantita'] < 50:
                    return ['background-color: #ffcccc'] * len(row)
                return [''] * len(row)
            
            st.write("Giacenze attuali (in rosso le quantità sotto la soglia di 50 unità)")
            st.dataframe(df_giacenze.style.apply(highlight_low_inventory, axis=1), use_container_width=True)
            
            # Opzioni di esportazione
            if st.button("Esporta Giacenze in Excel"):
                excel_data = converti_a_excel(df_giacenze)
                st.download_button(
                    label="Download Excel",
                    data=excel_data,
                    file_name="giacenze_materiali.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
            
            # Modifica materiali esistenti
            with st.expander("Modifica Materiali Esistenti", expanded=False):
                materiale_da_modificare = st.selectbox(
                    "Seleziona materiale da modificare:",
                    options=[f"{item['codice']} - {item['nome']}" for item in st.session_state.device_giacenze]
                )
                
                codice_selezionato = materiale_da_modificare.split(" - ")[0] if materiale_da_modificare else ""
                
                if codice_selezionato:
                    # Trova il materiale selezionato
                    materiale = next((item for item in st.session_state.device_giacenze if item["codice"] == codice_selezionato), None)
                    
                    if materiale:
                        col1, col2, col3 = st.columns(3)
                        with col1:
                            st.text_input("Codice Prodotto", value=materiale["codice"], disabled=True)
                        with col2:
                            nuovo_nome = st.text_input("Nome Prodotto", value=materiale["nome"], key="mod_nome")
                        with col3:
                            nuova_quantita = st.number_input("Quantità", min_value=0, value=materiale["quantita"], key="mod_quantita")
                        
                        if st.button("Aggiorna Materiale"):
                            # Aggiorna il materiale
                            for i, item in enumerate(st.session_state.device_giacenze):
                                if item["codice"] == codice_selezionato:
                                    st.session_state.device_giacenze[i]["nome"] = nuovo_nome
                                    st.session_state.device_giacenze[i]["quantita"] = nuova_quantita
                                    break
                            
                            st.success(f"Materiale '{nuovo_nome}' aggiornato con successo!")
                            salva_dati_sessione()
                            
                # Elimina materiali
                materiale_da_eliminare = st.selectbox(
                    "Seleziona materiale da eliminare:",
                    options=[f"{item['codice']} - {item['nome']}" for item in st.session_state.device_giacenze],
                    key="elimina_materiale"
                )
                
                codice_da_eliminare = materiale_da_eliminare.split(" - ")[0] if materiale_da_eliminare else ""
                
                if codice_da_eliminare and st.button("Elimina Materiale"):
                    st.session_state.device_giacenze = [item for item in st.session_state.device_giacenze if item["codice"] != codice_da_eliminare]
                    st.success(f"Materiale '{materiale_da_eliminare}' eliminato!")
                    salva_dati_sessione()
        else:
            st.info("Nessun materiale presente in inventario. Aggiungi materiali utilizzando il form sopra o importali.")
    
    # Tab 2: Associazione materiali ai laboratori
    with tab2:
        st.subheader("Materiali Necessari per Laboratorio")
        st.write("In questa sezione puoi specificare quali materiali sono necessari per ciascun laboratorio.")
        
        if st.session_state.laboratori and st.session_state.device_giacenze:
            # Selezione del laboratorio
            lab_selezionato = st.selectbox(
                "Seleziona laboratorio:",
                options=[lab["nome"] for lab in st.session_state.laboratori]
            )
            
            if lab_selezionato:
                # Trova i requisiti esistenti per questo laboratorio
                requisiti_lab = [req for req in st.session_state.device_requisiti_lab if req.get("nome_lab") == lab_selezionato]
                
                # Form per aggiungere un nuovo requisito
                with st.expander("Aggiungi Materiale al Laboratorio", expanded=False):
                    # Crea liste di codici e nomi dei materiali disponibili
                    materiali_disponibili = [f"{item['codice']} - {item['nome']}" for item in st.session_state.device_giacenze]
                    
                    materiale_selezionato = st.selectbox(
                        "Seleziona materiale:",
                        options=materiali_disponibili
                    )
                    
                    codice_selezionato = materiale_selezionato.split(" - ")[0] if materiale_selezionato else ""
                    
                    # Quantità richiesta per il laboratorio
                    quantita_richiesta = st.number_input(
                        "Quantità richiesta per laboratorio:",
                        min_value=1,
                        value=1,
                        step=1
                    )
                    
                    if st.button("Aggiungi al Laboratorio"):
                        if codice_selezionato:
                            # Verifica se questo materiale è già associato al laboratorio
                            requisito_esistente = next((req for req in st.session_state.device_requisiti_lab 
                                                      if req.get("nome_lab") == lab_selezionato and req.get("codice_materiale") == codice_selezionato), None)
                            
                            if requisito_esistente:
                                # Aggiorna il requisito esistente
                                for i, req in enumerate(st.session_state.device_requisiti_lab):
                                    if req.get("nome_lab") == lab_selezionato and req.get("codice_materiale") == codice_selezionato:
                                        st.session_state.device_requisiti_lab[i]["quantita"] = quantita_richiesta
                                        break
                                
                                st.success(f"Requisito aggiornato per il materiale nel laboratorio '{lab_selezionato}'!")
                            else:
                                # Aggiungi nuovo requisito
                                nome_materiale = next((item["nome"] for item in st.session_state.device_giacenze if item["codice"] == codice_selezionato), "")
                                
                                st.session_state.device_requisiti_lab.append({
                                    "nome_lab": lab_selezionato,
                                    "codice_materiale": codice_selezionato,
                                    "nome_materiale": nome_materiale,
                                    "quantita": quantita_richiesta
                                })
                                
                                st.success(f"Materiale aggiunto al laboratorio '{lab_selezionato}'!")
                            
                            salva_dati_sessione()
                
                # Mostra i requisiti esistenti per questo laboratorio
                if requisiti_lab:
                    st.write(f"Materiali necessari per il laboratorio '{lab_selezionato}':")
                    
                    df_requisiti = pd.DataFrame([
                        {"Codice": req.get("codice_materiale", ""),
                         "Nome Materiale": req.get("nome_materiale", ""),
                         "Quantità Richiesta": req.get("quantita", 0)}
                        for req in requisiti_lab
                    ])
                    
                    st.dataframe(df_requisiti, use_container_width=True)
                    
                    # Opzione per eliminare un requisito
                    with st.expander("Elimina Materiale dal Laboratorio", expanded=False):
                        if requisiti_lab:
                            requisito_da_eliminare = st.selectbox(
                                "Seleziona materiale da eliminare:",
                                options=[f"{req.get('codice_materiale')} - {req.get('nome_materiale')}" for req in requisiti_lab]
                            )
                            
                            codice_da_eliminare = requisito_da_eliminare.split(" - ")[0] if requisito_da_eliminare else ""
                            
                            if codice_da_eliminare and st.button("Elimina Materiale dal Laboratorio"):
                                st.session_state.device_requisiti_lab = [
                                    req for req in st.session_state.device_requisiti_lab 
                                    if not (req.get("nome_lab") == lab_selezionato and req.get("codice_materiale") == codice_da_eliminare)
                                ]
                                
                                st.success(f"Materiale '{requisito_da_eliminare}' rimosso dal laboratorio '{lab_selezionato}'!")
                                salva_dati_sessione()
                else:
                    st.info(f"Nessun materiale associato al laboratorio '{lab_selezionato}'. Usa il form sopra per aggiungere materiali.")
        else:
            if not st.session_state.laboratori:
                st.warning("Non ci sono laboratori nel sistema. Vai prima alla sezione 'Generazione Gruppi' per creare laboratori.")
            if not st.session_state.device_giacenze:
                st.warning("Non ci sono materiali in inventario. Aggiungi materiali nella tab 'Giacenze'.")
    
    # Tab 3: Importazione materiali da Excel
    with tab3:
        st.subheader("Importa Materiali")
        st.write("In questa sezione puoi importare materiali da Excel in formato tabellare.")
        
        import_tab1, import_tab2 = st.tabs(["Importa da Copia/Incolla", "Importa da File Excel"])
        
        # Tab 1: Importa da copia/incolla
        with import_tab1:
            st.write("""
            Copia i dati da Excel e incollali qui sotto. 
            La tabella deve avere le colonne: **codice**, **nome**, **quantità**
            (o nomi simili come "code", "name", "qty", ecc.)
            """)
            
            excel_paste = st.text_area(
                "Incolla qui i dati da Excel (con intestazioni):",
                height=200
            )
            
            if st.button("Importa Dati"):
                if excel_paste.strip():
                    nuovi_prodotti, prodotti_aggiornati, errore = importa_giacenze_da_excel(excel_paste)
                    
                    if errore:
                        st.error(f"Errore nell'importazione: {errore}")
                    else:
                        st.success(f"Importazione completata: {nuovi_prodotti} nuovi prodotti, {prodotti_aggiornati} prodotti aggiornati")
                        salva_dati_sessione()
                else:
                    st.warning("Nessun dato da importare. Incolla i dati nella casella di testo.")
        
        # Tab 2: Importa da file Excel
        with import_tab2:
            st.write("""
            Carica un file Excel con i dati dei materiali.
            Il file deve avere le colonne: **codice**, **nome**, **quantità**
            (o nomi simili come "code", "name", "qty", ecc.)
            """)
            
            file_excel = st.file_uploader(
                "Carica file Excel con l'elenco dei materiali",
                type=['xlsx', 'xls']
            )
            
            if file_excel is not None:
                try:
                    df_materiali = pd.read_excel(file_excel)
                    
                    st.write("Anteprima del file caricato:")
                    st.dataframe(df_materiali.head(10))
                    
                    # Selezione delle colonne
                    colonne = df_materiali.columns.tolist()
                    
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        col_codice = st.selectbox("Seleziona colonna del Codice:", colonne)
                    with col2:
                        col_nome = st.selectbox("Seleziona colonna del Nome:", colonne)
                    with col3:
                        col_quantita = st.selectbox("Seleziona colonna della Quantità:", colonne)
                    ...
                    if st.button("Importa da Excel"):
                        ...
                        for _, row in df_materiali.iterrows():
                            try:
                                codice_excel = str(row[col_codice]).strip() if not pd.isna(row[col_codice]) else ""
                                nome_excel = str(row[col_nome]).strip() if not pd.isna(row[col_nome]) else ""
                                ...
                                try:
                                    quantita_excel = int(row[col_quantita]) if not pd.isna(row[col_quantita]) else 0
                                except:
                                    quantita_excel = 0
                        ...
                        st.success(f"Importazione completata: {nuovi} nuovi prodotti, {aggiornati} prodotti aggiornati")
                        salva_dati_sessione()
                except Exception as e:
                    st.error(f"Errore nell'importazione del file Excel: {str(e)}")
    ...
    def genera_programmazione_automatica():
        ...
        def programma_laboratori_stessa_giornata(labs, gruppo, tipo_gruppo, aule_disp):
            ...
            for data in date_da_considerare:
                ...
                for lab in labs:
                    # Verifica se il laboratorio ha date specifiche e se questa data è consentita
                    date_lab_specifiche = lab.get("date_disponibili", [])
